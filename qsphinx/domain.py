...
def _pseudo_parse_arglist(signode, arglist):
    ...
    # print "PSEUDO PARSING ARGLIST"
    paramlist = qnodes.desc_qparameterlist()
    stack = [paramlist]
    try:
        ...
        signode += qnodes.desc_qparameterlist()
...
class QObject(ObjectDescription):
    ...
    def handle_signature(self, sig, signode):
        ...
        * it is added to the full name (return value) if not present
        """
        # print "HANDLING SIGNATURE "+ sig
        m = q_sig_re.match(sig)
        # print "M.GROUPS "+str(m.groups())
        if m is None:
            # print "REGEX ERROR"
            raise ValueError
        name_prefix, name, arglist = m.groups()
        retann = None
        # print "REF CONTEXT "+str(self.env.ref_context)

        # determine module and class name (if applicable), as well as full name
        modname = self.options.get(
            'module', self.env.ref_context.get('q:module'))
        classname = self.env.ref_context.get('q:namespace')
        if classname:
            add_module = False
    ...
    def after_content(self):
        if self.clsname_set:
            self.env.ref_context.pop('q:namespace', None)
...
class QClasslike(QObject):
    ...
    def before_content(self):
        ...
        # print "BEFORE CONTENT FOR QClassLike "+ str(self.names)
        if self.names:
            self.env.ref_context['q:namespace'] = self.names[0][0]
...
class QClassmember(QObject):
    ...
    def before_content(self):
        ...
        if lastname and not self.env.ref_context.get('q:namespace'):
            self.env.ref_context['q:namespace'] = lastname.strip('.')
            self.clsname_set = True
...
class QXRefRole(XRefRole):
    def process_link(self, env, refnode, has_explicit_title, title, target):
        ...
        refnode['q:namespace'] = env.ref_context.get('q:namespace')
        ...
        # print "TITLE, TARGET: %s,%s" %(title,target)
        return title, target
...
class QDomain(Domain):
    ...
    def find_obj(self, env, modname, classname, name, type, searchmode=0):
        ...
        # if not name in ["list", "symbol", "integer", "short", "dictionary"]:
        #     print modname
        #     print  classname
        #     print  name
        #     print  type
        #     print  searchmode
        #     # print "objtypes"+ str(objtypes)
